...
## Deployment & Publishing (For Maintainers)
...
**Option A: Google Container Registry (recommended for Cloud Run)**

```bash
# Tag for GCR
docker tag python-executor gcr.io/YOUR-PROJECT-ID/python-executor:latest

# Push to GCR
docker push gcr.io/YOUR-PROJECT-ID/python-executor:latest
```

**Option B: Docker Hub (for public access)**
...
# Tag for Docker Hub
docker tag python-executor your-dockerhub-username/python-executor:latest

# Push to Docker Hub
docker push your-dockerhub-username/python-executor:latest
```
...
gcloud run deploy python-executor \
  --image gcr.io/YOUR-PROJECT-ID/python-executor:latest \
  --region us-central1 \
...
After deployment, update the **Service URL** at the top of this README.
